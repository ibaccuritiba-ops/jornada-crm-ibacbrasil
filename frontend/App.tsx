...
const LeadsView = () => {
  ...
  const companyPipelines = useMemo(() => pipelines.filter(p => p.companyId === currentUser?.companyId && !p.deletado && p.ativo), [pipelines, currentUser]);
  const companyUsers = useMemo(() => users.filter(u => u.companyId === currentUser?.companyId && u.ativo), [users, currentUser]);
  ...
  const availableStagesForNewLead = useMemo(() => {
    ...
    return stages.filter(s => !s.deletado && s.pipeline_id === newLead.pipeline_id).sort((a,b) => a.ordem - b.ordem);
  }, [stages, newLead.pipeline_id]);

  const stagesForAllocation = useMemo(() => {
    if (!selectedPipelineId) return [];
    return stages.filter(s => !s.deletado && s.pipeline_id === selectedPipelineId).sort((a,b) => a.ordem - b.ordem);
  }, [stages, selectedPipelineId]);

  const stagesForFilter = useMemo(() => stages.filter(s => !s.deletado && s.pipeline_id === filterPipeline).sort((a,b) => a.ordem - b.ordem), [stages, filterPipeline]);
